...
function _Eval(_Par0) {
    ...
    for (;;) {
        ...
        if (0 < _Xx &&
            Math.random() <=
                (1 + _Yx * _Yx) *
                    Math.exp((_Par0._Alpha - 1) * Math.log(_Xx / (_Par0._Alpha - 1)) -
                        _Par0._Sqrt * _Yx)) {
            return _Par0._Beta * _Xx;
        }
    }
...
function randomPickWithWeight(probs) {
    probs = probs.reduce(function (acc, p) {
        var last = acc[acc.length - 1];
        acc.push(last + p);
        return acc;
    }, [0]);
    var x = Math.random() * probs[probs.length - 1];
    return probs.findIndex(function (p) { return p > x; }) - 1;
}
/**
 * @template T
 * @param {T[]} targets
...
export function randomPick(targets, weights) {
    ...
    if (!Array.isArray(weights))
        throw new TypeError("".concat(weights, " is not an array"));
